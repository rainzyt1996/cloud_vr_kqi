...
class DataUtils:
    ...
    def extract_capture_info(self, filepath: str, print_on=False, to_csv_on=True):
        ...
        i = [0]
        error_list = []
        field_list = ['src', 'dst', 'id', 'flags', 'proto', 'len']
        field_list2 = ['ip.src', 'ip.dst', 'ip.id', 'ip.flags', 'ip.proto', 'ip.len']
        ...
        def get_packet_info(packet):
            ...
            if 'ip' in dir(packet):
                for field in field_list:
                    if field not in dir(packet.ip):
                        error_list.append([i[0], 'no field in ip'])
                        return
                info = [str(packet.ip.src),
                        str(packet.ip.dst),
                        str(packet.ip.id),
                        str(packet.ip.flags),
                        str(packet.ip.proto),
            ...
            elif 'wlan_aggregate' in dir(packet):
                if 'wlan_aggregate.a_mdsu.subframe' not in packet.wlan_aggregate._all_fields.keys():
                    error_list.append([i[0], 'no wlan_aggregate.a_mdsu.subframe'])
                    return
                if isinstance(packet.wlan_aggregate._all_fields['wlan_aggregate.a_mdsu.subframe'], dict):
                    ...
                    error_list.append([i[0], 'type wlan_aggregate.a_mdsu.subframe'])
                    return
                for subframe in a_msdu_subframe:
                    if 'ip' not in subframe.keys():
                        error_list.append([i[0], 'no ip in subframe'])
                    else:
                        out = True
                        for field in field_list2:
                            if field not in subframe['ip'].keys():
                                error_list.append([i[0], 'no field in subframe ip'])
                                out = False
                                break
                        if out:
                ...
                error_list.append([i[0], 'no ip and wlan_aggregate'])
        ...
        if to_csv_on:
            ...
            error_df = pd.DataFrame(data=error_list, columns=['index', 'error_log'])
            error_df.to_csv(path_or_buf=os.path.join(os.path.dirname(filepath), 'capture_error'), index=False)
